...
Recap:
1. ATLC tokens will need sent to the address of the Whitelisttokensale contract. Anything unsold can be transferred back out to admin executing the withdraw functions.
2. BuyTokens functions will take ETH/DAI and hold it in the address of the Whitelisttokensale contract or can be sent to an external wallet. The contract will then transfer appropriate amount of ATLC tokens out to the buyer based on the multiplier/divisor configured amounts.
3. Withdraw functions will send the DAI being held in the Whitelisttokensale contract address to the caller. In this case any Admin can be transferred the xDAI tokens. Same with Withdraw of ALTC tokens. All or portion can be sent to admin caller's wallet.


Recap again:
1. ATLC tokens will need sent to the address of the Whitelisttokensale contract.
2. BuyTokens6 (now called buyTokensHoldInContract) function call will take xDAI and hold it in the address of the Whitelisttokensale contract and it will send appropriate amount of ATLC tokens out to the buyer.
3. Withdraw and WithdrawAll functions will send the total xDAI being held in the Whitelisttokensale contract address to the caller. In this case any Admin can be transferred the xDAI tokens. In my Withdraw, I sent it to d537 whom I made an admin of the contract. Coincidentally, this same d537 account is an owner as well.
New BuyTokens7 (now called  buyTokensSendToWallet)  function call will take xDAI and immediately transfer it to the wallet that we've specified INSTEAD of holding it in the Whitelisttokensale contract directly. Then, it will send appropriate amount of ATLC tokens out to the buyer.
...
AddAdmin: 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4(added the whitelisttokensale contract owner as the admin)
